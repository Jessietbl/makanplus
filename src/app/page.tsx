      ...
      <section className="max-w-xl w-full bg-white rounded-lg shadow-lg p-6 space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-center">
            <button
              type="submit"
              disabled={loading}
              className={`px-6 py-2 bg-green-500 text-white rounded-full transition ${loading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-green-600'}`}
            >
        ...
        <div>
          ...
          {loading ? (
            ...
            content && <p className="text-center text-gray-800">{content}</p>
          )}
        </div>
      </section>
    </main>
